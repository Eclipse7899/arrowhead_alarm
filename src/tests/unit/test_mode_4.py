...
from arrowhead_alarm.api.mode_4_client import Mode4Client
...
@pytest.fixture
def client(credentials: LoginCredentials) -> Mode4Client:
    return Mode4Client(
        host="127.0.0.1",
        port=12345,
...
@pytest.fixture
def request_mock(client: Mode4Client) -> AsyncMock:
...
def test_mode() -> None:
    assert Mode4Client.mode == ProtocolMode.MODE_4


def test_delimiter() -> None:
    assert Mode4Client.delimiter == "\n\r"
...
async def test_arm_button_success(
    ...
    with patch(
        "arrowhead_alarm.api.mode_4_client.arm_button_command",
...
async def test_arm_user_pin_success(
    ...
    with patch(
        "arrowhead_alarm.api.mode_4_client.arm_user_command",
        return_value=command,
    ) as command_mock:
...
async def test_arm_as_user_success(
    ...
    with patch(
        "arrowhead_alarm.api.mode_4_client.arm_no_pin_command",
        return_value=command,
    ) as command_mock:
...
async def test_arm_area_success(
    ...
    with patch(
        "arrowhead_alarm.api.mode_4_client.arm_area_command_mode_4",
        return_value=command,
    ) as command_mock:
        result = await client.arm_area(area, mode)
...
async def test_arm_area_failure(
    ...
    error = RuntimeError("area arming failed")
    request_mock.return_value = failed_result(error)
    command = MagicMock()
    ...
    with patch(
        ...
        with pytest.raises(
            RuntimeError,
            match="area arming failed",
        ) as exc_info:
            await client.arm_area(
                5,
                ArmingMode.AWAY,
            )
...
async def test_disarm_success(
    ...
    with patch(
        "arrowhead_alarm.api.mode_4_client.disarm_user_command",
        return_value=command,
    ) as command_mock:
